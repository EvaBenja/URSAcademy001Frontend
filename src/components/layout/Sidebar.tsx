...
import {
  ...
  CheckSquare, Bell, ClipboardList, ShieldCheck,
} from 'lucide-react';
...
export default function Sidebar() {
  ...
  const { notifsNonLues, ventes, demandes } = useStore();
  if (!user) return null;
  ...
  const demandesEnAtt  = (demandes||[]).filter(d => d.statut === 'en_attente').length;

  const getBadge = (to: string): number | null => {
    if (to.includes('/gestionnaire/ventes'))    return enAttente > 0 ? enAttente : null;
    if (to.includes('/gestionnaire/demandes'))  return demandesEnAtt > 0 ? demandesEnAtt : null;
    if (to.includes('suivi'))                   return notifsNonLues > 0 ? notifsNonLues : null;
    if (to.includes('/livreur/livraisons'))     return ventes.filter(v=>v.statut==='notif_livreur').length || null;
    return null;
  };

  return (
    <aside style={{ width:240, minHeight:'100vh', background:'linear-gradient(180deg,#003785 0%,#0d1b3e 100%)', display:'flex', flexDirection:'column', position:'fixed', left:0, top:0, bottom:0, zIndex:50, boxShadow:'4px 0 20px rgba(0,55,133,0.25)' }}>
      ...
      <div style={{ padding:'20px 16px 16px', borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
        ...
        <p style={{ fontFamily:'Cormorant Garamond,serif', fontSize:11, color:'rgba(255,255,255,0.3)', letterSpacing:'0.5px', paddingLeft:46 }}>Gestion Produits & Livraison</p>
      </div>
        ...
        {items.map(({ label, to, Icon }) => {
          ...
          return (
            <NavLink key={to} to={to} end={to==='/dashboard'||to.endsWith('dashboard')}
              style={({ isActive }) => ({
                display:'flex', alignItems:'center', gap:10, padding:'9px 13px', borderRadius:8, marginBottom:2,
              ...
              {badge ? (
                <span style={{ background:'#e53e3e', color:'white', fontSize:10, fontWeight:700, borderRadius:'50%', minWidth:18, height:18, display:'flex', alignItems:'center', justifyContent:'center', padding:'0 3px' }}>
                  {badge > 9 ? '9+' : badge}
                </span>
